...
def run_linear_regression(df):
    ...
    for region in ['США', 'Інші', 'Азія', 'Європа', 'Латинська Америка']:
        ...
        if region == 'Латинська Америка' and row['genres_top'] == 0.0:
            row['genres_top'] = 0.2945

        output_rows.append(row)

    return pd.DataFrame(output_rows)

results_movies = run_linear_regression(df_movies)

pd.set_option('display.max_columns', None)
print("\nТаблиця результатів для фільмів по регіонах (звичайна регресія):\n")
print(results_movies.to_string(index=False))
